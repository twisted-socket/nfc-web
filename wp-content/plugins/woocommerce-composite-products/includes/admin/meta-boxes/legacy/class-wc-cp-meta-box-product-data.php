					...
					<div class="form-field">
						...
						<div class="bto_scenario_selector_inner" <?php echo $modifier === 'masked' ? 'style="display:none"' : ''; ?>><?php
							...
							if ( false === $use_ajax ) {
									...
									foreach ( $scenario_data[ 'component_data' ][ $component_id ] as $product_id_in_scenario ) {
										...
										if ( $product_id_in_scenario == -1 ) {
											...
											if ( $product_in_scenario->get_type() === 'variation' ) {
												...
												$selections_in_scenario[ $product_id_in_scenario ] = WC_CP_Helpers::get_product_title( $product_in_scenario, sprintf( _x( '%s &ndash; Any Variation', 'any product variation', 'woocommerce-composite-products' ), $product_in_scenario->get_title() ) );
											} else {
								...
								?><input type="hidden" id="bto_scenario_ids_<?php echo $id; ?>_<?php echo $component_id; ?>" name="bto_scenario_data[<?php echo $id; ?>][component_data][<?php echo $component_id; ?>]" class="wc-component-options-search" style="width: 75%;" data-include="<?php echo esc_attr( json_encode( array( 'composite_id' => $product_id, 'component_id' => $component_id ) ) ); ?>" data-limit="100" data-component_optional="<?php echo $component_data[ 'optional' ]; ?>" data-placeholder="<?php _e( 'Search for products &amp; variations&hellip;', 'woocommerce-composite-products' ); ?>" data-action="woocommerce_json_search_products_and_variations_in_component" data-multiple="true" data-selected="<?php
			...
			<div class="form-field">
				...
				if ( ! empty( $component_options ) ) {
					...
					if ( false === $use_ajax ) {
						...
						?><input type="hidden" id="group_default_<?php echo $id; ?>" name="bto_data[<?php echo $id; ?>][default_id]" class="wc-product-search" style="width: 75%;" data-include="<?php echo esc_attr( json_encode( array( 'composite_id' => $product_id, 'component_id' => $component_id ) ) ); ?>" data-limit="100" data-placeholder="<?php _e( 'Search for a product&hellip;', 'woocommerce-composite-products' ); ?>" data-allow_clear="true" data-action="woocommerce_json_search_products_in_component" data-multiple="false" data-selected="<?php
