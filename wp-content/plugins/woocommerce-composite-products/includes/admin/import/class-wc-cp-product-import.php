...
class WC_CP_Product_Import {
	...
	public static function map_columns( $options ) {
		...
		$options[ 'wc_cp_hide_shop_price' ]           = __( 'Composite Shop Price Hidden', 'woocommerce-composite-products' );
	...
	public static function add_columns_to_mapping_screen( $columns ) {
		...
		$columns[ __( 'Composite Shop Price Hidden', 'woocommerce-composite-products' ) ]         = 'wc_cp_hide_shop_price';
		...
		if ( ! empty( $parsed_data[ 'wc_cp_components' ] ) ) {
			...
			if ( is_array( $components_rest_data ) ) {
				...
				foreach ( $components_rest_data as $component_rest_data ) {
					...
					$component_id = $component_rest_data[ 'id' ];
					...
					// Sanitize.
					$parsed_data[ 'wc_cp_components' ][ $component_id ] = WC_CP_REST_API::sanitize_rest_api_component_data( $parsed_component_data );
				}
			}
		}
		...
		if ( ! empty( $parsed_data[ 'wc_cp_scenarios' ] ) ) {
			...
			if ( is_array( $scenarios_rest_data ) ) {
				...
				foreach ( $scenarios_rest_data as $scenario_rest_data ) {
					...
					$scenario_id = $scenario_rest_data[ 'id' ];
					...
					// Sanitize.
					$parsed_data[ 'wc_cp_scenarios' ][ $scenario_id ] = WC_CP_REST_API::sanitize_rest_api_scenario_data( $parsed_scenario_data );
				}
			}
		}
