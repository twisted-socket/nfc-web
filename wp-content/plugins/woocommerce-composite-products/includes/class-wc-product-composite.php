...
class WC_Product_Composite extends WC_Product {
	...
	private $extended_data = array(
		'hide_shop_price'           => false,     // "Hide Price" option.
		'layout'                    => 'default', // "Composite Layout" option.
		'editable_in_cart'          => false,     // "Allow editing in cart" option.
	...
	public function load_defaults() {
		...
		$this->suppress_range_price_html = false;
	...
	public function sync_composite() {
		 ...
		 * @param  WC_Product_Composite  $this
		 */
		$update_raw_price_meta = apply_filters( 'woocommerce_composite_update_price_meta', $raw_price_meta_changed, $this ) && ! defined( 'WC_CP_UPDATING' );
		...
		if ( $this->contains( 'priced_individually' ) ) {
			...
			if ( isset( $this->composite_price_cache[ $cache_key ] ) ) {
				...
				if ( '' === $this->$raw_price_fn_name() || INF === $this->$raw_price_fn_name() ) {
					...
					$price = (double) wc_get_price_including_tax( $this, array( 'qty' => $qty, 'price' => $this->get_price() ) );
		...
		if ( $this->contains( 'priced_individually' ) ) {
			...
			if ( isset( $this->composite_price_cache[ $cache_key ] ) ) {
				...
				if ( '' === $this->$raw_price_fn_name() || INF === $this->$raw_price_fn_name() ) {
					...
					$price = (double) wc_get_price_excluding_tax( $this, array( 'qty' => $qty, 'price' => $this->get_price() ) );
		...
		if ( $this->contains( 'priced_individually' ) && ! empty( $components ) ) {
			...
			if ( $this->hide_price_html() || '' === $this->get_composite_price( 'min' ) ) {
				...
				if ( $suppress_range_format ) {
					...
					if ( $this->get_composite_price( 'min', true ) !== $this->get_composite_price( 'max', true ) ) {
						$price    = wc_format_price_range( $this->get_composite_price( 'min', true ), $this->get_composite_price( 'max', true ) );
						$is_range = true;
					} else {
						$price = wc_price( $this->get_composite_price( 'min', true ) );
					}
					...
					if ( $this->get_composite_regular_price( 'min', true ) !== $this->get_composite_price( 'min', true ) || $this->get_composite_regular_price( 'max', true ) > $this->get_composite_price( 'max', true ) ) {
						...
						if ( $this->get_composite_regular_price( 'min', true ) !== $this->get_composite_regular_price( 'max', true ) ) {
							$regular_price = wc_format_price_range( $this->get_composite_regular_price( 'min', true ), $this->get_composite_regular_price( 'max', true ) );
						} else {
							$regular_price = wc_price( $this->get_composite_regular_price( 'min', true ) );
						}
	...
	*/
	...
	public function get_hide_shop_price( $context = 'any' ) {
		return $this->get_prop( 'hide_shop_price', $context );
	}
	...
	*/
	 ...
	 * @return string
	 */
	public function set_hide_shop_price( $hide_shop_price ) {
		$hide_shop_price = wc_string_to_bool( $hide_shop_price );
		$this->set_prop( 'hide_shop_price', $hide_shop_price );
	}
	...
	*/
	...
	public function hide_price_html() {
		return apply_filters( 'woocommerce_composite_hide_price_html', $this->get_hide_shop_price(), $this );
	}
